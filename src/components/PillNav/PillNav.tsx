...
const PillNav: React.FC<PillNavProps> = ({
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      scrollTimeout.current = setTimeout(() => {
        ...
        for (const item of sectionItems) {
          ...
          if (section) {
            ...
            if (scrollPosition >= sectionTop && scrollPosition < sectionBottom) {
              newActiveHref = item.href;
              break;
            }
          }
        }

        // If no section found, check which section is closest to the viewport
        if (!newActiveHref && sectionItems.length > 0) {
        ...
        if (newActiveHref && currentActiveHref !== newActiveHref) {
          setCurrentActiveHref(newActiveHref);
          // Update URL hash without triggering navigation
          if (location.pathname === '/') {
            window.history.replaceState(null, '', newActiveHref);
          }
        }
      }, 100);
    };
    
    // Add scroll event listener only if we're on the home page
    if (location.pathname === '/') {
  ...
  }, [items, location.pathname, currentActiveHref]);
